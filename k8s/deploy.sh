...
if !  k3s kubectl get nodes &>/dev/null; then
  ...
  echo "👉 Please start k3s with:  systemctl start k3s"
  exit 1
fi
...
# Ensure namespace exists (idempotent)
 k3s kubectl create namespace library-dev --dry-run=client -o yaml |  k3s kubectl apply -f -

# Apply ConfigMaps and Secrets if they exist
[ -f ./dev/configmap.yaml ] &&  k3s kubectl apply -f ./dev/configmap.yaml -n library-dev
[ -f ./dev/secrets.yaml ]   &&  k3s kubectl apply -f ./dev/secrets.yaml   -n library-dev
...
if [ ! -f "$DEPLOYMENT_FILE" ]; then
 ...
 k3s kubectl apply -f "$DEPLOYMENT_FILE" -n library-dev
...
if [ -z "$ACTUAL_DEPLOYMENT_NAME" ]; then
  ACTUAL_DEPLOYMENT_NAME=$( k3s kubectl get deployment -n library-dev \
...
# Restart rollout to ensure fresh pods with latest image
 k3s kubectl rollout restart deployment/"$ACTUAL_DEPLOYMENT_NAME" -n library-dev

# Wait for rollout to complete
 k3s kubectl rollout status deployment/"$ACTUAL_DEPLOYMENT_NAME" -n library-dev --timeout=5m

echo "✅ Done! Your app should now be deployed to k3s in namespace: library-dev"

# Checking pods
echo "🔍 Checking pods..."
 k3s kubectl get pods -n library-dev